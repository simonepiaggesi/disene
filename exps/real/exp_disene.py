...
parser.add_argument('--feats', default=False, type=bool,
                  help='whether to include node features')
...
def main():
    ...
    for SEED in range(params['runs']):
        ...
        train_graph = to_networkx(train_data, to_undirected=True, remove_self_loops=True)
        ...
        for D_in in [128]: #dimensionality Z
            for D_out in [128]: #dimensionality H

                ####### Training

                if params['feats']:
                    EMB_FOLDER = '../../output/real/%s-walks-%s-%dhops-%dwindow/'%(MODEL.lower(), 'features', params['k_hops'], params['window_size'])
                else:
                    EMB_FOLDER = '../../output/real/%s-walks-%s-%dhops-%dwindow/'%(MODEL.lower(), 'featureless', params['k_hops'], params['window_size'])
