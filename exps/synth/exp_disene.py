...
from utils import *
from models import train_isgc_emb, train_imlp_emb
...
parser.add_argument('--model', default="isgc", type=str,
                  help='input model')
#'isgc' (DiSe-GAE), 'imlp' (DiSe-FCAE)

parser.add_argument('--k-hops', default=1, type=int,
                  help='number of conv. layers')

parser.add_argument('--window-size', default=5, type=int,
                  help='Window size of node2vec model')
...
def main():
    ...
    for SEED in range(params['runs']):
        ...
        train_graph = to_networkx(train_data, to_undirected=True, 
        ...
        #giant component
        giant = sorted(nx.connected_components(train_graph), key=len, reverse=True)[0]
        train_graph = train_graph.subgraph(giant)
        ...
        for D_in in [int(2**x) for x in range(3, 10)]: #dimensionality Z
            for D_out in [128]: #dimensionality H

                ####### Training

                EMB_FOLDER = '../../output/synth/%s-walks-%s-%dhops-%dwindow/'%(MODEL.lower(), 'featureless', params['k_hops'], params['window_size'])
                os.makedirs(EMB_FOLDER, exist_ok=True)
                emb_file = EMB_FOLDER+'%s_%s_%ddims_%ddims.%d.npz'%(DATASET, MODEL, D_in, D_out, SEED)

                if not os.path.isfile(emb_file):
                    if MODEL=='ISGC':
                        Z = train_isgc_emb(train_data, (train_data.x.shape[1], D_in, D_out), k_hops=params['k_hops'], window_size=params['window_size'])
                    if MODEL=='IMLP':
                        Z = train_imlp_emb(train_data, (train_data.x.shape[1], D_in, D_out), k_hops=params['k_hops'], window_size=params['window_size'])
                    np.savez_compressed(emb_file, Z)
                    print(DATASET, MODEL, D_in, D_out, SEED, 'embeddings saved')
                ...
                scores_file = METRIC_FOLDER + '%s_%s_%ddims_%ddims.%d.scores.npz'%(DATASET, MODEL, D_in, D_out, SEED)
                masks_file = METRIC_FOLDER + '%s_%s_%ddims_%ddims.%d.masks.npz'%(DATASET, MODEL, D_in, D_out, SEED)
                ...
                spath_file = METRIC_FOLDER + '%s_%s_%ddims_%ddims.%d.spath.npz'%(DATASET, MODEL, D_in, D_out, SEED)
                ...
                task_file = METRIC_FOLDER + '%s_%s_%ddims_%ddims.%d.linear_task.npz'%(DATASET, MODEL, D_in, D_out, SEED)
                ...
                if not os.path.isfile(task_file):
                    ...
                    scores_file = METRIC_FOLDER + '%s_%s_%ddims_%ddims.%d.linear_scores.npz'%(DATASET, MODEL, D_in, D_out, SEED)
                    masks_file = METRIC_FOLDER + '%s_%s_%ddims_%ddims.%d.linear_masks.npz'%(DATASET, MODEL, D_in, D_out, SEED)
                    ...
                    print(DATASET, MODEL, D_in, D_out, SEED, 'task metrics saved')
