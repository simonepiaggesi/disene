    ...
    for SEED in range(params['runs']):
        ...
        for D_in in [int(2**x) for x in range(3, 10)]:
            for D_out in [128]:
                ...
                EMB_FOLDER = '../../output/synth/%s+dine-%s/'%(MODEL.lower(), 'featureless')
                IN_FOLDER = '../../output/synth/%s-%s/'%(MODEL.lower(), 'featureless')
                ...
                task_file = METRIC_FOLDER + '%s_%s_%ddims.DINE_%ddims.%d.linear_task.npz'%(DATASET, MODEL, D_in, D_out, SEED)
                ...
                if not os.path.isfile(task_file):
                    ...
                    explainer = shap.LinearExplainer(clf, Z_train[y_train==1])#shap.LinearExplainer(clf, Z_train)
                    shap_values = explainer(Z_true)
                    emb_masks = shap_values.values.T

                    np.savez_compressed(task_file, **{k:i_task[k] for k in i_task.keys()})

                    scores_file = METRIC_FOLDER + '%s_%s_%ddims.DINE_%ddims.%d.linear_scores.npz'%(DATASET, MODEL, D_in, D_out, SEED)
                    masks_file = METRIC_FOLDER + '%s_%s_%ddims.DINE_%ddims.%d.linear_masks.npz'%(DATASET, MODEL, D_in, D_out, SEED)
                    #if not os.path.isfile(scores_file):
